...
class ExcelComparisonApp:
    ...
    def on_tab_changed(self, event):
        """Handle tab change events: populate comboboxes and preview for the active sheet"""
        try:
            ...
            id1_combo = tab_widgets['id1_combo']
            amount1_combo = tab_widgets['amount1_combo']
            id2_combo = tab_widgets['id2_combo']
            amount2_combo = tab_widgets['amount2_combo']
            ...
            for combo in [id1_combo, amount1_combo, id2_combo, amount2_combo]:
                combo['values'] = columns  # set available options
            
            # Restore previous selections if they exist
            if sheet_name in self.column_selections:
                selections = self.column_selections[sheet_name]
                id1_combo.set(selections.get('id1', ''))
                amount1_combo.set(selections.get('amount1', ''))
                id2_combo.set(selections.get('id2', ''))
                amount2_combo.set(selections.get('amount2', ''))
            else:
                # Clear comboboxes for new sheet
                for combo in [id1_combo, amount1_combo, id2_combo, amount2_combo]:
                    combo.set('')
            
            # --- Load data into Treeview preview ---
            tree.delete(*tree.get_children())
            tree["columns"] = columns
            tree["show"] = "headings"
            
            # Collect first 100 rows for preview
            data = []
            ...
            for idx, col in enumerate(columns):
                max_width = max(len(str(row[idx])) if idx < len(row) else 0 for row in data)
                max_width = max(max_width, len(col))
                pixel_width = max(50, min(max_width * 8 + 20, 300))  # min 50, max 300 px
            ...
            wb.close()
            self.status_var.set(f"Loaded sheet '{sheet_name}' successfully. Select columns as needed.")
    ...
    def on_column_selected(self, sheet_name):
        """Handle when the user selects columns in a tab"""
        ...
        # Save selections for this sheet
        self.column_selections[sheet_name] = current_selections

        # Cascade propagation to all following sheets
        if all(current_selections.values()):
            self.propagate_to_following_tabs(sheet_name, current_selections)

        # Update process button state
        self.update_process_button_state()


    def propagate_column_selections(self, source_selections):
        """Copy column selections from first sheet into other sheets as defaults"""
        for other_sheet in self.workbook.sheetnames:
            if other_sheet == self.workbook.sheetnames[0]:  # skip first sheet
                continue

            if other_sheet in self.tab_frames:
                tab_widgets = self.tab_frames[other_sheet]

                # Apply defaults to comboboxes
                for key, combo in [
                    ('id1', tab_widgets['id1_combo']),
                    ('amount1', tab_widgets['amount1_combo']),
                    ('id2', tab_widgets['id2_combo']),
                    ('amount2', tab_widgets['amount2_combo'])
                ]:
                    if source_selections.get(key):
                        combo.set(source_selections[key])

                # Save propagated selections as valid
                self.column_selections[other_sheet] = source_selections.copy()

    def propagate_to_following_tabs(self, source_sheet_name, source_selections):
        """
        Propagate column selections from source_sheet_name to all sheets after it in tab order.
        Only affects sheets after the source; previous sheets are not touched.
        """
        sheet_names = self.workbook.sheetnames
        source_index = sheet_names.index(source_sheet_name)

        # Loop over following sheets
        for other_sheet in sheet_names[source_index + 1:]:
            if other_sheet in self.tab_frames:
                tab_widgets = self.tab_frames[other_sheet]

                # Get available columns for this sheet
                ws = load_workbook(self.filepath, data_only=True)[other_sheet]
                max_col = ws.max_column
                ...
                for key, combo in [
                    ...
                    combo['values'] = available_columns  # update options
                    if source_selections.get(key) in available_columns:
                        combo.set(source_selections[key])
    ...
    def update_process_button_state(self):
        """Enable process button if at least one sheet is fully configured"""
        self.process_btn['state'] = 'disabled'
